...
The source which we'll be using to get our avatar is [DiceBear Avatars](https://avatars.dicebear.com/) api.
...
– get the value from an external api using while using user input as San argument in the api call.
...
If you've taken a look at the official tutorials, you've seen them using React class components. To me, React class component seems a little outdated, that's because I will use a functional component. Also, showing how to use functional component as Sanity input might be one of the rationales fot this post.

There is one little drawback regarding using functional components as Sanity input widgets. When using class component, it is sufficient to add a focus() method, but with functional one, we need to wrap the component in a forwardRef call and to invoke useImperativeHandle hook inside of it (with an object containing focus method as argument). It might be a little heavy-handed, but I would still go for it.
